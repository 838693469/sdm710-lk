...
#Extb850688 [BSP boot], LK info, liulai@wt add begin, 2018/11/22
WT_DATE = $(shell date +%Y%m%d_%H%M%S)
WT_BUILD_TIME=\"$(WT_DATE)\"
LK_VERSION = $(shell git rev-parse --verify --short HEAD)
WT_LK_VERSION=\"$(LK_VERSION)\"
...
#Extb850688 [BSP boot], LK info, liulai@wt add end, 2018/11/22
...
ABL_FV_IMG: EDK_TOOLS_BIN
	...
	-D WT_ATO_FACTORY_BUILD=$(WT_ATO_FACTORY_BUILD) \
	-D WT_BUILD=$(WT_BUILD) \
	-D WT_BUILD_TIME=$(WT_BUILD_TIME) \
	-D WT_LK_VERSION=$(WT_LK_VERSION) \
